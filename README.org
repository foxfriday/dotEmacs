...
Region to process from:
https://emacs.stackexchange.com/questions/37887/send-region-to-shell-in-another-buffer

#+begin_src emacs-lisp
(defun mar-region-to-process (arg beg end)
  "Send the current region to a process buffer.
On first call it prompts for the buffer name. Subsequent calls send to the same
buffer, unless a prefix argument is used (C-u), or the buffer no longer has an
active process."
  (interactive "P\nr")
  (if (or arg
...
(define-key evil-visual-state-map ",r" 'mar-region-to-process)
#+end_src
